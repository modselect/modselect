...
import { Button, Card, Text, Flex, Checkbox, Container, TextArea, Separator, Tabs, RadioGroup } from '@radix-ui/themes';
...
// import whiteSquare from '@/components/cards/white_square.jpeg';
...
const Img2ImgPanel = ({ SDConfig, setSDConfig, AppData }) => {
    ...
    const startPolling = () => {
        setIsPolling(true);
        timer = setInterval(pollingCallback, 500);
    };
    const stopPolling = () => {
        setIsPolling(false);
        clearInterval(timer);
            ...
            <Flex direction={"column"} gap={"3"} width={"100%"}>
                <Flex direction={"row"} gap={"2"} justify={"between"}>
                    ...
                    <Flex direction={"column"} gap={"2"}>
                        <Button onClick={() => runAPICall('/sdapi/v1/img2img')} size={"3"}>
                            Generate
                        </Button>
                        ...
                        <Card>
                            <Flex direction={"row"} gap={"2"} width={"100%"} align={"start"}>
                                <Flex direction={"column"} gap={"4"} width={"100%"}>
                                    <Flex direction={"column"} gap={"2"}>
                                        <ImageSelector SDConfig={SDConfig} setSDConfig={setSDConfig} />
                                        <Flex direction={"column"} gap={"1"}>
                                            <Text size={"1"}>Resize mode</Text>
                                            <Flex direction={"row"} gap={"1"}>
                                                <RadioGroup.Root
                                                    value={String(SDConfig.resize_mode)}
                                                    onValueChange={(e) => setSDConfig({ ...SDConfig, resize_mode: Number(e) })}>
                                                    <Flex gap="1" direction="row" justify={"start"}>
                                                        <Text align="center" as="label" size="1" style={{ border: "1px solid lightgray", borderRadius: "4px", padding: "2px" }}>
                                                        ...
                                                        <Text align="center" as="label" size="1" style={{ border: "1px solid lightgray", borderRadius: "4px", padding: "2px" }}>
                                                            <Flex gap="1">
                                                                <RadioGroup.Item value="1" /> Crop and resize
                                                            </Flex>
                                                        </Text>
                                                        <Text align="center" as="label" size="1" style={{ border: "1px solid lightgray", borderRadius: "4px", padding: "2px" }}>
                                                            <Flex gap="1">
                                                                <RadioGroup.Item value="2" /> Resize and fill
                                                            </Flex>
                                                        </Text>
                                                        <Text align="center" as="label" size="1" style={{ border: "1px solid lightgray", borderRadius: "4px", padding: "2px" }}>
                                                            <Flex gap="1">
                                                                <RadioGroup.Item value="3" /> Just resize (latent upscale)
                                                            </Flex>
                                                        </Text>
                                                    </Flex>
                                                </RadioGroup.Root>
